...
box_group_plot <- function(
  ...
  plot <- plot +
    ggplot2::geom_vline(
      ggplot2::aes(
        xintercept = mean(as.numeric(.df[[xvar]]), na.rm = TRUE),
        color = "Overall mean",
    ...
    ggplot2::geom_vline(
      ggplot2::aes(
        xintercept = stats::median(as.numeric(.df[[xvar]]), na.rm = TRUE),
    ...
    ggplot2::scale_y_discrete(
      labels = rlang::as_function(~ stringr::str_c(.x, "\nn = ", counts[.x]))
    ) + # add in the n counts
